...
class LivingThing:
    ...
    def move(self, tile, direction):
        if not tile.check_move(self.x, self.y, direction):
            logger.debug(
                self.wm.stdscr,
                f"Living thing attempted to move {direction}; there is no path",
            )
        elif direction == "n":
            self.y -= 1
            return True
...
class Game:
    ...
    def combat(self, hostile: NPC):
        ...
        while in_combat:
            ...
            Utils.printline(
                self.wm.stdscr, f"You: {self.player.hp}/{self.player.max_hp} HP"
            )
            Utils.printline(
                self.wm.stdscr,
                f"You can {Utils.color_string('melee', Fore.RED)} attack, or attempt to {Utils.color_string('run', Fore.CYAN)}.",
            )
            try:
            ...
            if ui in ["melee", "m"]:
                ...
                Utils.printline(
                    self.wm.stdscr, f"You take a swing at the {enemy_text}!"
                )
                dmg_txt = Utils.color_string(f"{act_dmg} damage", Fore.RED)
                Utils.printline(
                    self.wm.stdscr, f"You do ({min_dmg}-{max_dmg}) {dmg_txt}!"
                )
    ...
    def map_turn(self):
        ...
        if ct_npc.is_dead:
            ...
            if ct_npc.will_attack_player():
                ...
                Utils.printline(
                    self.wm.stdscr, f"There is a friendly {ct_npc.name} in this room!"
                )
                Utils.printline(
                    self.wm.stdscr, "Non-hostile NPC encounters not yet implemented."
                )
        self.current_tile.print_map(self.player.x, self.player.y)
        Utils.printline(
            self.wm.stdscr, "What direction do you want to move? [n/e/s/w] "
        )
        ...
        if command in ["n", "e", "s", "w"]:
            ...
            self.wm.mapscr.clear()
            Utils.printline(
                self.wm.stdscr, "You take the portal into the next dimension..."
            )
        ...
        # beyond here lies debug commands
            ...
            Utils.printline(
                self.wm.stdscr, 'teleport to what coordinates? (i.e. "1, 3")'
            )
    ...
    def play(self):
        ...
        while True:
            ...
            self.wm.mapscr.refresh()
            Utils.printline(
                self.wm.stdscr, "You are in the hub world. Go to 'map' or 'portal' pls."
            )
            # input_field.getch()
            command = Utils.get_input(self.wm.input_scr)
            if command == "map":
